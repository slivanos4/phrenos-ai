...
function scoreHeroCandidate(story: HeroCandidate): number {
  ...
  if (tags.includes("eye-opening")) score += 8;
  if (tags.includes("agentic")) score += 3;
  if (tags.includes("enterprise")) score += 3;
  if (tags.includes("regulation") || tags.includes("safety")) score += 2;
  if (tags.includes("multimodal") || tags.includes("research")) score += 2;
  if (story.section === "products_industry") score += 1;
  ...
  // Prefer earlier curated stories slightly (research agent tends to lead with strength).
...
async function pickHeroWithClaude(stories: HeroCandidate[]): Promise<HeroCandidate | null> {
...
Avoid the weakest "nice to know" item.
...
Return ONLY JSON: {"story_id":"<id>","reason":"one short sentence"}`;
  ...
  try {
    ...
    const match = stories.find((story) => story.id === parsed.story_id);
    return match ?? null;
  } catch {
    return null;
  }
...
async function loadHeroCandidates(runId: string): Promise<HeroCandidate[]> {
  ...
  const { data: sources } = await supabase
    .from(SOURCES_TABLE)
    .select("story_id, extracted_facts")
  ...
  return stories.map((story) => {
    ...
    return {
      ...
      summary_html: (story.summary_html as string | null) ?? null,
...
export async function generateWeekHeroContent(runId: string): Promise<{
  ...
  const candidates = await loadHeroCandidates(runId);
  ...
  try {
    const chosen = await pickHeroWithClaude(candidates);
