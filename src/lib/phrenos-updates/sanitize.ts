...
export function sanitizePlainText(text: string): string {
  return text
    .split(/\n{2,}/)
    .map((paragraph) => sanitizeDashes(paragraph.replace(/\n/g, " ").trim()))
    .filter(Boolean)
    .join("\n\n");
}
...
export function parseSummaryBullets(value: string): string[] {
  ...
  if (/<ul[\s>]/i.test(value)) {
    const fromList = [...value.matchAll(/<li[^>]*>([\s\S]*?)<\/li>/gi)]
      .map((match) => sanitizeDashes(stripHtmlToText(match[1]).replace(/\s+/g, " ").trim()))
      .filter(Boolean);
  ...
  for (const rawLine of text.split(/\n/)) {
    ...
    if (/^[-•]\s+/.test(line)) {
      bullets.push(sanitizeDashes(line.replace(/^[-•]\s+/, "").trim()));
      continue;
    }
...
export function bulletsToSummaryText(bullets: string[]): string {
  return bullets
    .map((bullet) => sanitizeDashes(bullet.replace(/^[-•]\s*/, "").trim()))
    .filter(Boolean)
    .map((bullet) => `- ${bullet}`)
    .join("\n");
}
